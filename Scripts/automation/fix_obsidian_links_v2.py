...
def fix_links_in_wiki():
    ...
    # Get list of actual wiki filenames
    wiki_files = set()
        ...
        if file.endswith('.md') and file not in ['index.md', 'log.md']:
            ...
            try:
                ...
                def fix_link(match):
                    ...
                    if link_text in wiki_files:
                        return f"[[{link_text}]]"
                    # Try with .pdf extension
                    if link_text + '.pdf' in wiki_files:
                        return f"[[{link_text}.pdf]]"
                    # Try with .pdf.md extension
                    if link_text + '.pdf.md' in wiki_files:
                        return f"[[{link_text}.pdf.md]]"
