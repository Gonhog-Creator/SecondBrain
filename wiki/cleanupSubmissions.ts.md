...
Source: junk_drawer/github/PersonalWebsite/scripts/cleanupSubmissions.ts.txt

Category: [[github-code]]

## Summary
import { withRedis } from '@/lib/kv'; async function cleanupSubmissions() { try { await withRedis(async (client) => { // Get all submission keys const keys = await client.keys('submission:*'); // Get all submissions const submissions = await Promise.all(
...
## Metadata
- Source file: junk_drawer/github/PersonalWebsite/scripts/cleanupSubmissions.ts.txt
