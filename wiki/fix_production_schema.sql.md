...
Source: junk_drawer/github/DeltaDash/backend/fix_production_schema.sql.txt

Category: [[github-code]]

## Summary
-- Run this SQL directly on your production database to fix the immediate schema issues -- Add missing columns to ammunition table ALTER TABLE ammunition ADD COLUMN IF NOT EXISTS name VARCHAR NOT NULL DEFAULT ''; ALTER TABLE ammunition ADD COLUMN IF NOT EXISTS caliber_unit VARCHAR; ALTER TABLE ammunition ADD COLUMN IF NOT EXISTS caliber_diameter_mm NUMERIC(10,3); ALTER TABLE ammunition ADD COLUMN IF NOT EXISTS caliber_length_mm NUMERIC(10,3); ALTER TABLE ammunition ADD COLUMN IF NOT EXISTS calib
...
## Metadata
- Source file: junk_drawer/github/DeltaDash/backend/fix_production_schema.sql.txt
